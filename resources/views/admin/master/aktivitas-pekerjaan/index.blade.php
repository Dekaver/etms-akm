        ...
        <div class="page-title">
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Aktivitas Pekerjaan</li>
                </ol>
            </nav>
        </div>
        <div class="page-btn">
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Add Aktivitas Pekerjaan </h5>
        ...
        <script>
            $(function() {
                var table = $('table.data-table').DataTable({
                    processing: true,
                    serverSide: false,
                    ajax: "{{ route('aktivitas-pekerjaan.index') }}",
                    columns: [{
                            data: 'DT_RowIndex',
                            name: 'DT_RowIndex',
                            orderable: false,
            ...
            $('#form-modal').on('show.bs.modal', function(event) {
                ...
                if (post == 'new') {
                    ...
                    modal.find('form').attr('action', `{{ route('aktivitas-pekerjaan.store') }}`)
                } else {
                    var id = button.data('id');
                    $.ajax({
                        method: "GET",
                        url: `{{ route('aktivitas-pekerjaan.index') }}/${id}/edit`
                    }).done(function(response) {
                        modal.find('input[name="nama"]').val(response.nama);
                        modal.find('input[name="_method"]').val('PUT');
                    });
                    modal.find('form').attr('action', `{{ route('aktivitas-pekerjaan.index') }}/${id}`)
                }
            });
