        ...
        <script>
            ...
            $('#form-modal').on('show.bs.modal', function(event) {
                ...
                if (post == 'new') {
                    ...
                    modal.find('form').attr('action', `{{ route('department.store') }}`)
                } else {
                    var id = button.data('id');
                    $.ajax({
                        method: "GET",
                        url: `{{ route('department.index') }}/${id}/edit`
                    }).done(function(response) {
                        modal.find('input[name="department"]').val(response.department);
                        modal.find('input[name="_method"]').val('PUT');
                    });
                    modal.find('form').attr('action', `{{ route('department.index') }}/${id}`)
                }
            });
