                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="form-group">
                                    <label>Size<span class="manitory">*</span></label>
        ...
        <script>
            ...
            $('#form-modal').on('show.bs.modal', function(event) {
                ...
                if (post == 'new') {
                    ...
                    modal.find('form').attr('action', `{{ route('size.store') }}`)
                } else {
                    var id = button.data('id');
                    $.ajax({
                        method: "GET",
                        url: `{{ route('size.index') }}/${id}/edit`
                    }).done(function(response) {
                        modal.find('input[name="name"]').val(response.name);
                        modal.find('input[name="_method"]').val('PUT');
                    });
                    modal.find('form').attr('action', `{{ route('size.index') }}/${id}`)
                }
            });
